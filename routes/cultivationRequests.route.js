...
router.get('/cultivation-requests/:id', verifyJWT, async (req, res) => {
  try {
    ...
    if (!request) {
      return res.status(404).json({ error: 'Cultivation request not found' });
    }
    res.json(request);
  } catch (error) {
    res.status(500).json({ error: error.message });
...
router.put('/cultivation-requests/:id/progress', verifyJWT, async (req, res) => {
  try {
    ...
    const request = await CultivationRequest.findOneAndUpdate(
      { _id: req.params.id, farmerId: req.user.uid },
      {
        ...
        ...(req.body.actualEndDate && { actualEndDate: req.body.actualEndDate })
      },
      { new: true }
    );
